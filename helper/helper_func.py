...
async def get_message_id(client, message):
    """Private links aur forwarded posts se safely Message ID aur Channel ID nikalne ke liye update"""
    if message.forward_from_chat:
        ...
        # Private (/c/) aur public links dono nikalne ka regex pattern
        pattern = r"https://t.me/(?:c/)?([\w_-]+)/(\d+)"
        matches = re.match(pattern, text_content)
        if not matches:
            return 0, 0
        ...
        if channel_id.isdigit():
            ...
            if f"-100{channel_id}" == str(client.db) or f"-{channel_id}" == str(client.db) or channel_id == str(client.db):
                return msg_id, client.db
            # Check secondary DB channels
            db_channels = getattr(client, 'db_channels', {})
            for channel_id_str in db_channels.keys():
                clean_db_str = channel_id_str.replace("-100", "").replace("-", "")
                if channel_id == clean_db_str:
                    return msg_id, int(channel_id_str)
            # Safe Fallback to prevent NoneType crash
            return msg_id, client.db
        else:
...
async def get_messages_from_db_channels(client, temb_ids):
    ...
    try:
        ...
        msgs = await client.get_messages(chat_id=primary_db, message_ids=temb_ids)
        ...
        for channel_id_str, channel_data in db_channels.items():
            ...
            try:
                additional_msgs = await client.get_messages(chat_id=int(channel_id_str), message_ids=missing_ids)
...
def is_user_subscribed(statuses):
    return all(status in {ChatMemberStatus.MEMBER, ChatMemberStatus.ADMINISTRATOR, ChatMemberStatus.OWNER} for status in statuses.values() if status is not None) and bool(statuses)
...
def force_sub(func):
    async def wrapper(client: Client, message: Message):
        ...
        if photo:
            msg = await message.reply_photo(caption="<b>ᴡᴀɪᴛ ᴀ sᴇᴄᴏɴᴅ.....</b>", photo=photo)
        else:
            msg = await message.reply("<code><b>ᴡᴀɪᴛ ᴀ sᴇᴄᴏɴᴅ.....</b></code>")
...
def convert_time(duration_seconds: int) -> str:
    periods = [('Yᴇᴀʀ', 60 * 60 * 24 * 365), ('Mᴏɴᴛʜ', 60 * 60 * 24 * 30), ('Day', 60 * 60 * 24), ('Hour', 60 * 60), ('Minute', 60), ('Second', 1)]
    parts = []
...
async def batch_auto_del_notification(bot_username, msg, delay_time, transfer): 
    ...
    try:
        if transfer:
            ...
            button = [[InlineKeyboardButton(text="• ɢᴇᴛ ғɪʟᴇs •", url=link), InlineKeyboardButton(text="ᴄʟᴏsᴇ •", callback_data = "close")]]
            await temp.edit_text(text=f"<b>›› Pʀᴇᴠɪᴏ Uz Mᴇssᴀɢᴇ ᴡᴀs Dᴇʟᴇᴛᴇᴅ\n\nIғ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ɢᴇᴛ ᴛʜᴇ ғɪʟᴇs ᴀɢᴀɪɴ, ᴛʜᴇɴ ᴄʟɪᴄᴋ: <a href={link}>• ɢᴇᴛ ғɪʟᴇs •</a> ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ.</b>", reply_markup=InlineKeyboardMarkup(button), disable_web_page_preview = True)
        else:
            await temp.edit_text(f"<b>Pʀᴇᴠɪᴏ Us Mᴇssᴀɢᴇ ᴡᴀs Dᴇʟᴇᴛᴇᴅ </b>")
    except Exception:
        pass
